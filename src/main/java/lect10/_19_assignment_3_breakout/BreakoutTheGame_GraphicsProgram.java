...
public class BreakoutTheGame_GraphicsProgram extends GraphicsProgram {
	...
	private void startBallMovement() {
		...
		while (true) {
			...
			// if the collider is brick, then brick will be removed and score will be increased
			...
			if(collider!=null){
				if(collider == paddle || collider == scoreLabel){
					...
					// To solve this I am thinking to add below statements, that will check the collision with paddle
					// whenever the paddle is moved, and if the collision is found then it will make sure that the ball goes upwards.
					if((ball.getY() + BALL_RADIUS * 2) > (getHeight() - PADDLE_Y_OFFSET - PADDLE_HEIGHT)){
						ball.move(vx, -((ball.getY() + BALL_RADIUS * 2) - (getHeight() - PADDLE_Y_OFFSET - PADDLE_HEIGHT)));
					}
					...
					// If all the bricks are finished, then stopping the loop and printing the success message
					if(bricksCount==0){
						createWinMessage();
						break;
					}
			...
			// we will remove the ball and place a new one at starting position
			...
			// also Ball is glued to the top wall, this code is fixing this issue
			if(y < 0){
				vy = -vy;
				ball.move(-vx, -y);
			}
			...
			// also Ball is glued to the left wall, this code is fixing this issue
			if(x<0){
				vx = -vx;
				ball.move(-x, -vy);
			}
	...
	private void createWinMessage() {
		...
		label.setLocation((getWidth()-label.getWidth())/2, NBRICK_ROWS * BRICK_HEIGHT+BRICK_SEP+BRICK_Y_OFFSET+label.getAscent());
		add(label);
	}

	/*
	 * This method will update the score board label, which is shown just below the paddle
	 * for showing the score to the player
	 */
	...
	/*
	 * This method will create a Game Over label at the center of screen, when player
	 * loses all his turns
	 */
	...
	private void createBall() {
		ball = new GOval((getWidth() - 2 * BALL_RADIUS) / 2,
				NBRICK_ROWS * BRICK_HEIGHT+BRICK_SEP+BRICK_Y_OFFSET, 2 * BALL_RADIUS, 2 * BALL_RADIUS);
		ball.setFilled(true);
	...
	public void mouseMoved(MouseEvent e) {
		...
		if(paddle!=null && scoreLabel!=null){
			if (e.getX() + PADDLE_WIDTH < getWidth()) {
				...
				scoreLabel.setLocation(e.getX()+(PADDLE_WIDTH-scoreLabel.getWidth())/2, (getHeight() - PADDLE_Y_OFFSET + PADDLE_HEIGHT + scoreLabel.getAscent()));
			}
		}
	}

	/* 
	 * This method will create a paddle at the bottom center of the window
	 * also will create a score board type label
	 * */
	private void createPaddle() {
		...
		scoreLabel.setLocation((getWidth() - scoreLabel.getWidth()) / 2, (getHeight() - PADDLE_Y_OFFSET + PADDLE_HEIGHT + scoreLabel.getAscent()));
